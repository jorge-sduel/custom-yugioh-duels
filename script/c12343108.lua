...
function s.initial_effect(c)
	...
	e1:SetTarget(s.target(TYPE_MONSTER))
	e1:SetOperation(s.activate(TYPE_MONSTER))
	c:RegisterEffect(e1)
	--search ST
	...
	e2:SetTarget(s.target(TYPE_SPELL+TYPE_TRAP))
	e2:SetOperation(s.activate(TYPE_SPELL+TYPE_TRAP))
	c:RegisterEffect(e2)
	--to hand
...
function s.thfilter(c,t)
	return c:IsSetCard(0x222) and c:IsType(t) and c:IsAbleToHand()
end
function s.target(t)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk)
		if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK,0,1,nil,t) end
		Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
	end
end
function s.activate(t)
	return	function(e,tp,eg,ep,ev,re,r,rp)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_DECK,0,1,1,nil,t)
		if #g>0 then
