...
function c888000034.penop(e,tp,eg,ep,ev,re,r,rp,c,og)
	local rpz=Duel.GetFieldCard(tp,LOCATION_SZONE,7)
	if c:GetSequence()==7 then rpz=Duel.GetFieldCard(tp,LOCATION_SZONE,6) end
	local lscale=c:GetLeftScale()
	local rscale=rpz:GetRightScale()
...
function c888000034.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x888)
end
function c888000034.damcon(e,tp,eg,ep,ev,re,r,rp)
	local seq=e:GetHandler():GetSequence()
	local tc=Duel.GetFieldCard(e:GetHandlerPlayer(),LOCATION_SZONE,13-seq)
	return tc and tc:IsSetCard(0x888) and Duel.IsExistingMatchingCard(c888000034.cfilter,tp,LOCATION_MZONE,0,2,nil)
end
