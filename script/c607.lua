...
function s.filter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x581) and c:GetFlagEffect(id)==0
		and Duel.IsExistingMatchingCard(Card.IsXyzSummonable,tp,LOCATION_EXTRA,0,1,nil,c)
end
...
function s.scop(e,tp,eg,ep,ev,re,r,rp)
	...
	if mc then
		...
		local sc=Duel.SelectMatchingCard(tp,Card.IsXyzSummonable,tp,LOCATION_EXTRA,0,1,1,nil,mc):GetFirst()
		Duel.XyzSummon(tp,sc,mc)
