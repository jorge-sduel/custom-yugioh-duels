...
--]]
--Constants
TYPE_RUNE			= TYPE_ACTION+TYPE_MONSTER
...
function Rune.Operation(monf,mmin,mmax,stf,smin,smax,group,customoperation,stage2)
	return 	function(e,tp,eg,ep,ev,re,r,rp,c,must,g,min,max)
				...
				if rmgroup then
					g:Sub(rmgroup)
			                Duel.ConfirmCards(1-tp,rmgroup)
...
--
	...
	e9:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e9:SetCode(EFFECT_CHANGE_LEVEL)
	e9:SetCondition(function(e) return not e:GetHandler():IsHasEffect(999381000) end)
	e9:SetValue(0)
	c:RegisterEffect(e9)
--
	local e10=Effect.CreateEffect(c)
	...
	e10:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e10:SetCode(EFFECT_ALLOW_NEGATIVE)
	e10:SetCondition(function(e) return not e:GetHandler():IsHasEffect(999381000) end)
	c:RegisterEffect(e10)
end
--add Level Rune monster and is Synchronable
--[[loc=Location card loc1 and loc2= Rune monster affected loc3 and loc4=Synchro Monster can use Rune monster as material]]
function Auxiliary.AddRuneTuning(c,loc,loc1,loc2,loc3,loc4)
	local e2=Effect.CreateEffect(c)
