...
function s.cfilter(c,e,tp,ft)
	...
	return c:GetOriginalType()&TYPE_MONSTER~=0 and lv>0 and c:IsFaceup() and c:IsAbleToGraveAsCost() and (ft>0 or c:GetSequence()<5)
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,lv,rc,att,e,tp) and c:IsSetCard(0x52)
end
function s.spfilter(c,lv,rc,att,e,tp)
	return c:IsLevel(lv) and c:IsSetCard(0x52) and c:IsCanBeSpecialSummoned(e,0,tp,true,true)
end
