...
function c99200161.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	Fusion.AddProcCodeFun(c,99200151,aux.FilterBoolFunction(Card.IsRace,RACE_MACHINE),2,true,true)
	--spsummon condition
	...
	e5:SetDescription(aux.Stringid(99200161,3))
...
function c99200161.spfilter1(c,tp)
	return c:IsCode(99200151) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(true)
		and Duel.IsExistingMatchingCard(c99200161.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,c)
end
...
function c99200161.sprcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3
		and Duel.IsExistingMatchingCard(c99200161.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp)
end
function c99200161.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200161,0))
	local g1=Duel.SelectMatchingCard(tp,c99200161.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200161,1))
	local g2=Duel.SelectMatchingCard(tp,c99200161.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,2,g1:GetFirst())
	g1:Merge(g2)
