...
function s.initial_effect(c)
	...
	Xyz.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_HERALDIC_BEAST),5,3)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
    	...
    	elseif op==2 then
        	...
        	local sc2=Duel.GetMatchingGroup(s.efffilter,tp,0,LOCATION_MZONE,c)
        	if sc and not sc:IsStatus(STATUS_DISABLED) then
            		local code=sc:GetOriginalCode()
            	
sc2:NegateEffects(c,RESETS_STANDARD_PHASE_END)
