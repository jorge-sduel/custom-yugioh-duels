...
--Space-Time summon
function Auxiliary.AddSpacetSummonProcedure(c,f,min,max,specialchk,opp,loc,send)
	if loc==nil then loc=LOCATION_MZONE end
...
function Spacet.GetSpacetCount(c)
    if c:GetAttack()>1 then return c:GetAttack() end
    return 1
end
...
function Spacet.CheckGoal(tp,sg,lc,minc,f,specialchk,filt)
	...
	return #sg>=minc and sg:CheckWithSumEqual(Spacet.GetBigbangCount,lc:GetAttack(),#sg,#sg)
		and (not specialchk or specialchk(sg,lc,SUMMON_TYPE_SPECIAL,tp)) and Duel.GetLocationCountFromEx(tp,tp,sg,lc)>0
end
