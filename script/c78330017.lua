...
function c78330017.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.IsExistingMatchingCard(c78330017.filter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(78330017,1)) then
		Duel.PayLPCost(tp,500)
		e:SetLabel(1)
	end
end
function c78330017.filter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xac9812) and not c:IsType(TYPE_TUNER) and not c:IsType(TYPE_SYNCHRO) and not c:IsType(TYPE_XYZ) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
