...
function cid.filter(c,e,tp,m)
	local cd=c:GetCode()
	if not (c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,true,true) and (c:IsType(TYPE_RITUAL) and c:IsMonster()) or (c:GetOriginalType()==TYPE_PENDULUM&TYPE_RITUAL and c:IsLocation(LOCATION_PZONE) and c:GetOriginalLevel()>0)) then return false end
