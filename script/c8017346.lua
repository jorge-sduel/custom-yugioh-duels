...
function cid.filter(c,e,tp,m)
	local cd=c:GetCode()
	if not (c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,true,true) and (c:IsLocation(LOCATION_EXTRA+LOCATION_PZONE) or c:IsFaceup()) and c:IsType(TYPE_RITUAL) and c:IsMonster() or c:IsLocation(LOCATION_PZONE)) then return false end
