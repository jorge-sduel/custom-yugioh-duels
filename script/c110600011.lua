...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_HAND+LOCATION_PZONE,0,1,nil,e,tp,mg)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_HAND+LOCATION_PZONE,0,1,1,nil,e,tp,mg)
	if #tg>0 then
