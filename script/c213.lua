...
function c213.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(213,1))
	e2:SetType(EFFECT_TYPE_ACTIVATE)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetRange(LOCATION_SZONE)
	e2:SetTarget(c213.target)
	e2:SetOperation(c213.activate)
	c:RegisterEffect(e2)
--Reduce damage
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(213,2))
	...
	e4:SetDescription(aux.Stringid(213,3))
	e4:SetType(EFFECT_TYPE_ACTIVATE)
	e4:SetCode(EVENT_FREE_CHAIN)
	e4:SetRange(LOCATION_HAND)
	e4:SetTarget(c213.target)
	e4:SetOperation(c213.activate)
