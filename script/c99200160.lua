...
function c99200160.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	Fusion.AddProcCodeFun(c,99200151,aux.FilterBoolFunction(Card.IsRace,RACE_MACHINE),1,true,true)
	--spsummon condition
	...
	e4:SetDescription(aux.Stringid(99200160,3))
...
function c99200160.spfilter1(c,tp)
	return c:IsCode(99200151) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(true)
		and Duel.IsExistingMatchingCard(c99200160.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,c)
end
...
function c99200160.sprcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2
		and Duel.IsExistingMatchingCard(c99200160.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp)
end
function c99200160.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200160,0))
	local g1=Duel.SelectMatchingCard(tp,c99200160.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200160,1))
	local g2=Duel.SelectMatchingCard(tp,c99200160.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,g1:GetFirst())
	g1:Merge(g2)
	...
	Duel.SendtoGrave(g1,nil,1,REASON_COST)
end
function c99200160.immfilter(e,te)
	return te:IsActiveType(TYPE_TRAP) and te:GetOwnerPlayer()~=e:GetHandlerPlayer()
end
function c99200160.spfilter(c,e,tp)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xda3790)
