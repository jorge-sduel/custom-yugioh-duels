...
function(c,reg,desc)
	...
	e1:SetCondition(RPendulum.Condition())
	e1:SetOperation(RPendulum.Operation())
...
function RPendulum.Filter(c,e,tp,lscale,rscale,lvchk)
	...
	return (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM)))
		and (lvchk or not (lv>lscale and lv<rscale) or c:IsHasEffect(511004423)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_PENDULUM,tp,false,false)
		and not c:IsForbidden()
end
function RPendulum.Condition()
	return	function(e,c,ischain,re,rp)
				...
				return g:IsExists(RPendulum.Filter,1,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
			end
end
function RPendulum.Operation()
	return	function(e,tp,eg,ep,ev,re,r,rp,c,sg,inchain)
				...
				local tg=nil
				if og then
					tg=og:Filter(Card.IsLocation,nil,loc):Filter(RPendulum.Filter,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
				else
					tg=Duel.GetMatchingGroup(RPendulum.Filter,tp,loc,0,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
				end
				...
				while true do
					...
					if sg:IsContains(tc) then
						...
						if c:IsHasEffect(511007000)~=nil or rpz:IsHasEffect(511007000)~=nil then
							if not RPendulum.Filter(tc,e,tp,lscale,rscale) then
								...
								local pg=sg:Filter(aux.NOT(RPendulum.Filter),nil,e,tp,lscale,rscale)
								sg:Sub(pg)
								if #pg>0 then
