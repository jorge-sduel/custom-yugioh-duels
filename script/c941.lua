...
function s.spcon(e,c)
	...
	return Duel.CheckReleaseGroup(1-c:GetControler(),Card.Type,2,false,2,true,c,1-c:GetControler(),nil,false,nil,TYPE_MONSTER) and Duel.GetFieldGroupCount(c:GetControler(),0,LOCATION_MZONE)>=2 and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc	and c:IsRelateToBattle() then
		local lv=tc:GetRank()
		local e1=Effect.CreateEffect(c)
