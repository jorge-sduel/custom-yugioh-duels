...
function c888000032.penfilter(c,e,tp,lscale,rscale)
	local lv=c:GetLevel()
	return (c:IsLocation(LOCATION_HAND) or c:IsFaceup()) and c:IsType(TYPE_RITUAL)
		and lv>lscale and lv<rscale and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_PENDULUM,tp,true,false)
...
function c888000032.pencon(e,c,og)
	...
	if c:GetSequence()==7 then rpz=Duel.GetFieldCard(tp,LOCATION_SZONE,6) end
...
function c888000032.penop(e,tp,eg,ep,ev,re,r,rp,c,og)
	local rpz=Duel.GetFieldCard(tp,LOCATION_SZONE,7)
	if c:GetSequence()==7 then rpz=Duel.GetFieldCard(tp,LOCATION_SZONE,6) end
	local lscale=c:GetLeftScale()
	local rscale=rpz:GetRightScale()
...
function c888000032.splimit(e,c,sump,sumtype,sumpos,targetp)
	return not c:IsType(TYPE_RITUAL) and (bit.band(sumtype,SUMMON_TYPE_PENDULUM)==SUMMON_TYPE_PENDULUM or bit.band(sumtype,SUMMON_TYPE_RITUAL)==SUMMON_TYPE_RITUAL)
end
...
function c888000032.acfilter(c,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsType(TYPE_QUICKPLAY+TYPE_TRAP) and (not condition or condition(te,tp,eg,ep,ev,re,r,rp)) and (not cost or cost(te,tp,eg,ep,ev,re,r,rp,0))
...
function c888000032.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 and Duel.IsExistingMatchingCard(c888000032.acfilter,tp,0x13,0,1,nil,tp,eg,ep,ev,re,r,rp) end
