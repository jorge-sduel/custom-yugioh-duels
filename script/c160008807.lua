...
function cid.initial_effect(c)
	--synchro summon
	Synchro.AddProcedure(c,nil,1,1,Synchro.NonTuner(Card.IsSetCard,0xab5),1,99)
	c:EnableReviveLimit() 
...
function cid.filter(c,e,tp)
	return c.Is_Evolute and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function cid.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) end
	...
	local ct=Duel.GetMatchingGroupCount(cid.filter2,tp,LOCATION_MZONE,0,e:GetHandler())
