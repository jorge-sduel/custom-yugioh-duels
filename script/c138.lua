...
function c138.filter(c,e,tp,m)
	...
	if 
		--cd~=27000302 or 
		not (((c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,true,false) and not c:IsLocation(LOCATION_EXTRA)) or (c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,true,true) and c:IsLocation(LOCATION_EXTRA) and c:IsFaceup())) and c:IsType(TYPE_RITUAL) and c:IsMonster()) then return false end
