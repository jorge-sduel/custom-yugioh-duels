...
s.Is_Neutrino=true
	--fusion material
	Fusion.AddProcFunRep(c,s.matfilter,2,true)
	c:EnableReviveLimit()
	--damage
...
function s.damfilter(c,fc,tp)
	return (c.Is_Neutrino or c:IsRace(RACE_DRAGON)) and c:GetBaseAttack()>0 and c:IsAbleToGrave()
end
...
function s.spfilter(c,e,tp)
	return c.Is_Neutrino or c:IsRace(RACE_DRAGON) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
