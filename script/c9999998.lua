...
function c9999998.initial_effect(c)
	...
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,79856792,89631139)
	--spsummon condition
...
function c9999998.spfilter(c,e,tp)
	return not c:IsCode(9999998) and c:IsType(TYPE_FUSION)
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,true,false) 
end
function c9999998.ffilter(c,e)
	return c:IsType(TYPE_MONSTER) and not c:IsImmuneToEffect(e) and not c:IsCode(9999998)
end
function c9999998.sptg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(c9999998.spfilter,tp,LOCATION_EXTRA,LOCATION_EXTRA,1,nil,e,tp) end
...
function c9999998.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c9999998.spfilter,tp,LOCATION_EXTRA,LOCATION_EXTRA,1,1,nil,e,tp)
	local tc=g:GetFirst()
