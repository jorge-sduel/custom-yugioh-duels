...
if not REVERSEPENDULUM_IMPORTED then Duel.LoadScript("proc_reverse_pendulum.lua") end   
   ...
   c:AddSetcodesRule(id,false,0xbb00)
	...
	e5:SetCondition(cid.sprcon)
	e5:SetOperation(cid.sprop)
	e5:SetValue(SUMMON_TYPE_FUSION)
	c:RegisterEffect(e5)
...
function cid.penop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_PZONE)>0 then
