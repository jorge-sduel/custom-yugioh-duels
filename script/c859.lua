...
--over
	...
	--e0:SetCost(c859.hspcost)
	e0:SetCondition(c859.hspcon)
	e0:SetOperation(c859.hspop)
	c:RegisterEffect(e0)
	--cannot be destroyed
...
function c859.cvfilter(c)
	return c:IsSetCard(0x5DC) and c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
...
function c859.hspcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c859.hspcon(e,c)
	...
	return Duel.IsExistingMatchingCard(c859.cvfilter,tp,LOCATION_HAND,0,1,nil) and Duel.GetLocationCount(e:GetHandlerPlayer(),LOCATION_MZONE)+Duel.GetLocationCountFromEx(e:GetHandlerPlayer(),e:GetHandlerPlayer(),nil,c)>0
...
function c859.hspop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g2=Duel.SelectMatchingCard(tp,c859.cvfilter,tp,LOCATION_HAND,0,1,1,nil)
