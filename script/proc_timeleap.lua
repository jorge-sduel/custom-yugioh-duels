...
function Auxiliary.AddTimeleapProcedure(c,code,loc,excon)
	...
	e1:SetTarget(Auxiliary.TleapSummonTarget)
	e1:SetOperation(Auxiliary.TleapSummonOperation)
	c:RegisterEffect(e1)
end
...
function Auxiliary.TleapSummonOperation(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local sg=e:GetLabelObject()
	Duel.Remove(sg,POS_FACEUP,REASON_MATERIAL+REASON_TIMELEAP)
	c:SetMaterial(sg)
	sg:DeleteGroup()
