...
--Reunion Summon
function Timeleap.AddProcedure(c,f,min,max,cond,specialchk,opp,loc,send)
	...
	e1:SetCondition(Timeleap.Condition(f,min,max,cond,specialchk,opp,loc,send))
...
function Timeleap.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg=e:GetHandler():GetMaterial()
      --[[mg:AddMonsterAttribute(TYPE_MONSTER)
	...
	e2:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_STANDBY+RESET_SELF_TURN,99)
	e2:SetCountLimit(1)
	...
	e3:SetOperation(Timeleap.resetcount)
	e3:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END+RESET_OPPO_TURN,99)
	c:RegisterEffect(e3)
	c:SetTurnCounter(0)
...
function Timeleap.resetcount(e,tp,eg,ep,ev,re,r,rp)
	Timeleap.spop2(e:GetLabelObject(),tp,eg,ep,ev,e,r,rp)
end
...
function Auxiliary.AddTimeleapProcedure(c,cd,loc,excon)
	...
	e1:SetTarget(Auxiliary.Timeleaphsptg(cd,loc))
	e1:SetOperation(Auxiliary.Timeleaphspop(cd,loc))
	c:RegisterEffect(e1)
	--Special summon
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
