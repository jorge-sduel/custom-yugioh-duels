...
function s.fusfilter(c,e,tp,fe)
	return c:IsType(TYPE_FUSION) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) 
		...
		and Duel.GetLocationCountFromEx(tp,fe,nil,c)>2
end
...
function s.xyzfilter(c,e,tp,fc,fe)
	return c:IsType(TYPE_XYZ) and c:IsRankBelow(4) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false)
end
function s.penfilter(c,e,tp,fc,fe)
	return (c:IsType(TYPE_PENDULUM) and c:IsFaceup()) or (c:IsType(TYPE_PENDULUM) and not c:IsLocation(LOCATION_EXTRA)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local xc=Duel.SelectMatchingCard(tp,s.xyzfilter,tp,LOCATION_EXTRA,0,1,1,fc,e,tp,fc,e):GetFirst()
	local pc=Duel.SelectMatchingCard(tp,s.penfilter,tp,LOCATION_EXTRA+LOCATION_DECK+LOCATION_HAND,0,1,1,fc,e,tp,fc,e):GetFirst()
...
Duel.SpecialSummonStep(pc,0,tp,tp,false,false,POS_FACEUP)
	Duel.SpecialSummonComplete()
	fc:CompleteProcedure()
	sc:CompleteProcedure()
	xc:CompleteProcedure()
	pc:CompleteProcedure()
end
