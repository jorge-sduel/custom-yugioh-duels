...
function s.thfilter1(c,ctype)
	return c.is_armor and c:IsType(ctype) and c:IsAbleToHand()
end
function s.cfilter1(c)
	if not c.is_armor or c:IsPublic() then return false end
...
function s.cost1(e,tp,eg,ep,ev,re,r,rp)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter1,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONFIRM)
	local g=Duel.SelectMatchingCard(tp,s.cfilter1,tp,LOCATION_HAND,0,1,1,e:GetHandler())
...
function s.thfilter2(c)
	return c.is_armor and c:IsAbleToHand()
end
