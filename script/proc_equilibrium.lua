...
function Equilibrium.Filter(c,e,tp,lscale,rscale,lvchk)
	...
	return (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM)))
		and (lvchk or (lv>lscale and lv<rscale) or c:IsHasEffect(511004423)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_EQUILIBRIUM,tp,false,false)
...
function Equilibrium.Condition()
	return	function(e,c,ischain,re,rp)
				...
				if Duel.GetLocationCountFromEx(tp,tp,nil,TYPE_EQUILIBRIUM)>0 then loc=loc+LOCATION_EXTRA end
				...
				return g:IsExists(Equilibrium.Filter,1,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
			end
end
function Equilibrium.Operation()
	return	function(e,tp,eg,ep,ev,re,r,rp,c,sg,inchain)
				...
				local ft2=Duel.GetLocationCountFromEx(tp,tp,nil,TYPE_EQUILIBRIUM)
				...
				local tg=nil
				if og then
					tg=og:Filter(Card.IsLocation,nil,loc):Filter(Equilibrium.Filter,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
				else
					tg=Duel.GetMatchingGroup(Equilibrium.Filter,tp,loc,0,nil,e,tp,lscale,rscale,c:IsHasEffect(511007000) and rpz:IsHasEffect(511007000))
				end
				...
				while true do
					...
					if sg:IsContains(tc) then
						...
						if c:IsHasEffect(511007000)~=nil or rpz:IsHasEffect(511007000)~=nil then
							if not Equilibrium.Filter(tc,e,tp,lscale,rscale) then
								...
								local pg=sg:Filter(aux.NOT(Equilibrium.Filter),nil,e,tp,lscale,rscale)
								sg:Sub(pg)
								if #pg>0 then
