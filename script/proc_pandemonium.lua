PANDEMONIUM_IMPORTED=true
if not aux.PandemoniumProcedure then
	aux.PandemoniumProcedure = {}
	Pandemonium = aux.PandemoniumProcedure
end
if not Pandemonium then
	Pandemonium = aux.PandemoniumProcedure
end
--add procedure to Pendulum monster, also allows registeration of activation effect
Pandemonium.AddProcedure = aux.FunctionWithNamedArgs(
function(c,reg,desc)
		local e1=Effect.CreateEffect(c)
 		e1:SetDescription(69,3)
 		e1:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
 		e1:SetRange(LOCATION_PZONE)
 		e1:SetCondition(Pandemonium.Condition)
 		e1:SetOperation(Pandemonium.Operation)
	        local e2=Effect.CreateEffect(c)
	        e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_GRANT)
	        e2:SetRange(LOCATION_MZONE)
	        e2:SetTargetRange(LOCATION_PZONE,0)
	        e2:SetTarget(Pandemonium.Efpendulum)
	...
	e3:SetCondition(Pandemonium.Condition())
	e3:SetOperation(Pandemonium.Operation())
	...
	if reg==nil or reg then
		local e4=Effect.CreateEffect(c)
 		...
 		e4:SetCondition(Pandemonium.Condition)
 		e4:SetOperation(Pandemonium.Operation)
	        c:RegisterEffect(e4)
	end

end,"handler","register","desc")

function Pandemonium.Filter(c,e,tp,lscale,rscale,lvchk)
