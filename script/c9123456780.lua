...
local s,id=GetID()
local TYPES=TYPE_FUSION|TYPE_RITUAL|TYPE_SYNCHRO|TYPE_XYZ|TYPE_PENDULUM|TYPE_LINK|TYPE_PENDULUM
...
function s.rescon(sg,e,tp,mg)
	...
	return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA+LOCATION_MZONE+LOCATION_GRAVE,0,1,nil,e,tp,sg:GetFirst():GetOriginalRace())
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then 
		local g=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_EXTRA+LOCATION_MZONE+LOCATION_GRAVE,0,nil)
		return aux.SelectUnselectGroup(g,e,tp,4,6,s.rescon,0)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_EXTRA+LOCATION_MZONE+LOCATION_GRAVE,0,nil)
	local rg=aux.SelectUnselectGroup(g,e,tp,4,6,s.rescon,1,tp,HINTMSG_REMOVE)
