...
c249000235.is_reverse_xyz=true
...
function c249000235.drcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousPosition(POS_FACEUP) and e:GetHandler():IsPreviousLocation(LOCATION_ONFIELD) and not e:GetHandler():IsPreviousLocation(LOCATION_PZONE)
end
...
function c249000235.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if (dc:IsRace(RACE_SPELLCASTER) or dc:IsRace(RACE_FAIRY)) and Duel.IsPlayerCanDraw(tp,1)
		...
		if Duel.GetFieldGroupCount(tp,LOCATION_PZONE,0)>0 
			and Duel.SelectYesNo(tp,aux.Stringid(249000235,4)) then
			Duel.BreakEffect()
