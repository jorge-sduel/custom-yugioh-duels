...
function c68.ffilter(c,e,tp,lscale,rscale,lvchk)
	...
	return (c:IsLocation(LOCATION_HAND) or (c:IsFaceup() and c:IsType(TYPE_PENDULUM)))
		and (lvchk or (lv>lscale and lv<rscale) or c:IsHasEffect(511004423)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_PENDULUM,tp,true,true)
		and not c:IsForbidden() and (c:IsType(TYPE_MONSTER) c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_PENDULUM,tp,true,true) or (c:IsType(TYPE_SPELL) or c:IsType(TYPE_TRAP)))
end
