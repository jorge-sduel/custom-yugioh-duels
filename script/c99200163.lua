...
function c99200163.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	Fusion.AddProcCodeFun(c,99200151,aux.FilterBoolFunction(Card.IsRace,RACE_MACHINE),4,true,true)
	--spsummon condition
	...
	e5:SetDescription(aux.Stringid(99200163,3))
...
function c99200163.spfilter1(c,tp)
	return c:IsCode(99200151) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial(true)
		and Duel.IsExistingMatchingCard(c99200163.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,4,c)
end
...
function c99200163.sprcon(e,c)
	...
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-5
		and Duel.IsExistingMatchingCard(c99200163.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp)
end
function c99200163.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200163,0))
	local g1=Duel.SelectMatchingCard(tp,c99200163.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99200163,1))
	local g2=Duel.SelectMatchingCard(tp,c99200163.spfilter2,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,4,4,g1:GetFirst())
	g1:Merge(g2)
	...
	Duel.SendtoGrave(g1,nil,4,REASON_COST)
end
