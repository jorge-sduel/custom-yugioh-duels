...
function Evolute.IsLocation(c,e,loc)
	...
	return (c:IsLocation(loc) or c:IsHasEffect(16000820,tp)) or  or c:IsFaceup()
end
function Evolute.ConditionFilter(c,f,lc,tp)
	return (not f or f(c,lc,SUMMON_TYPE_SPECIAL,tp) and (c:IsLocation(loc) or (c:IsLocation(LOCATION_HAND) or c:IsHasEffect(16000820,tp))) and not (c:IsType(TYPE_LINK) or c:IsHasEffect(50031787,tp))
end
