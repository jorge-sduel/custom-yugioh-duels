...
function Evolute.ConditionFilter(c,f,lc,tp)
	return (not f or f(c,lc,SUMMON_TYPE_SPECIAL,tp)) and ((c:IsLocation(LOCATION_MZONE) and c:IsFaceup()) or (c:IsHasEffect(16000820,tp) and c:IsLocation(LOCATION_HAND)))
end
...
function Auxiliary.EvoluteSummonFilter(c,cd)
	return ((cd and c:IsCode(cd)) or (not cd and c.Is_Evolute)) and c:IsAbleToRemoveAsCost()
end
...
function Auxiliary.EvoluteSummonOperation(cd,loc)
	return	function(e,tp,eg,ep,ev,re,r,rp,c)
				...
				Duel.SentToGrave(tc,REASON_COST)
				g:DeleteGroup()
			end
end
