...
function Evolute.IsLocation(c,e,tp,loc)
	if loc==nil then loc1=LOCATION_MZONE and loc2=LOCATION_HAND end
	if loc==LOCATION_HAND then loc1=LOCATION_HAND and loc2=LOCATION_MZONE end
	--if c:IsCode(221594325) then loc1=LOCATION_HAND end
	return (c:IsLocation(loc1) and (not c:IsLocation(loc2)) or c:IsHasEffect(16000820,tp)) and ((c:IsLocation(LOCATION_MZONE) and c:IsFaceup()) or c:IsLocation(LOCATION_HAND))
end
...
--Convergent evolute
function Auxiliary.AddConvergentEvolSummonProcedure(c,f,min,max,specialchk,desc)
...
function Auxiliary.ConvergentEvolSummonFilter(c,cd,lc)
	return ((not cd or cd(c,lc,SUMMON_TYPE_SPECIAL,tp)) or c.Is_Evolute) and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
--Duel.GetLocationCountFromEx(c:GetControler(),c:GetControler(),c,lc)>0
end
...
function Auxiliary.ConvergentEvolSummonTarget(cd,loc)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk,c)
				...
				local sg=aux.SelectUnselectGroup(g,e,tp,2,99,aux.ChkfMMZ(1),2,tp,HINTMSG_REMOVE,nil,nil,true)
				if #sg>0 then
					sg:KeepAlive()
					e:SetLabelObject(sg)
					return true
...
function Auxiliary.CEvCheckGoal(tp,sg,lc,minc,f,specialchk,filt)
	...
	return 
--#sg>=minc and sg:CheckWithSumEqual(Auxiliary.CEvGetCount,4,#sg,#sg) and 
