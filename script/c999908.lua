...
function c999908.drtarg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c999908.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
...
function c999908.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	local g=Duel.SelectMatchingCard(tp,c999908.spfilter,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
	if #g>0 then
		Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	Duel.Draw(p,d,REASON_EFFECT)
	end
end
function c999908.filter(c)
	return c:IsFaceup() and not c:IsType(TYPE_TUNER)
end
function c999908.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c999908.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c999908.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c999908.filter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c999908.operation(e,tp,eg,ep,ev,re,r,rp)
