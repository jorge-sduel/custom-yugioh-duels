...
condition if synchro summoned
    return e:GetHandler():GetSummonType()==SUMMON_TYPE_SPECIAL+SUMMON_TYPE_SYNCHRO
]]
--XyzSynchro Summon
function XyzSynchro.AddProcedure(c,f,min,max,specialchk,opp,loc,send)
	...
	e1:SetCondition(XyzSynchro.Condition(f,min,max,specialchk,opp,loc,send))
	e1:SetTarget(XyzSynchro.Target(f,min,max,specialchk,opp,loc,send))
	e1:SetOperation(XyzSynchro.Operation(f,min,max,specialchk,opp,loc,send))
...
function XyzSynchro.GetXyzSynchroCount(c)
...
function XyzSynchro.CheckRecursive(c,tp,sg,mg,lc,minc,maxc,f,specialchk,og,emt,filt)
...
function XyzSynchro.CheckRecursive2(c,tp,sg,sg2,secondg,mg,lc,minc,maxc,f,specialchk,og,emt,filt)
	if #sg>maxc then return false end
	sg:AddCard(c)
	for _,filt in ipairs(filt) do
		if not filt[2](c,filt[3],tp,sg,mg,lc,filt[1],1) then
	...
	if #(sg2-sg)==0 then
		if secondg and #secondg>0 then
			local res=secondg:IsExists(XyzSynchro.CheckRecursive,1,sg,tp,sg,mg,lc,minc,maxc,f,specialchk,og,emt,{table.unpack(filt)})
			sg:RemoveCard(c)
			return res
		else
			local res=XyzSynchro.CheckGoal(tp,sg,lc,minc,f,specialchk,{table.unpack(filt)})
			sg:RemoveCard(c)
			return res
		end
	end
	local res=XyzSynchro.CheckRecursive2((sg2-sg):GetFirst(),tp,sg,sg2,secondg,mg,lc,minc,maxc,f,specialchk,og,emt,filt)
	sg:RemoveCard(c)
	return res
end
function XyzSynchro.CheckGoal(tp,sg,lc,minc,f,specialchk,filt)
	for _,filt in ipairs(filt) do
	...
	return	function(e,c,must,g,min,max)
				...
				if res then
					if #mustg==max then
						...
						res=mustg:IsExists(XyzSynchro.CheckRecursive,1,sg,tp,sg,(mg+tg),c,min,max,f,specialchk,mg,emt)
					elseif #mustg<max then
						local sg=mustg
						res=(mg+tg):IsExists(XyzSynchro.CheckRecursive,1,sg,tp,sg,(mg+tg),c,min,max,f,specialchk,mg,emt)
					end
				end
...
function XyzSynchro.Target(f,minc,maxc,specialchk,opp,loc,send)
	return	function(e,tp,eg,ep,ev,re,r,rp,chk,c,must,g,min,max)
				...
				while #sg<max do
					local filters={}
					if #sg>0 then
						XyzSynchro.CheckRecursive2(sg:GetFirst(),tp,Group.CreateGroup(),sg,mg+tg,mg+tg,c,min,max,f,specialchk,mg,emt,filters)
					end
					local cg=(mg+tg):Filter(XyzSynchro.CheckRecursive,sg,tp,sg,(mg+tg),c,min,max,f,specialchk,mg,emt,{table.unpack(filters)})
					if #cg==0 then break end
					finish=#sg>=min and #sg<=max and XyzSynchro.CheckGoal(tp,sg,c,min,f,specialchk,filters)
...
function XyzSynchro.Operation(f,minc,maxc,specialchk,opp,loc,send)
