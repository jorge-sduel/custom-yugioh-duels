...
function s.initial_effect(c)
	...
	a2:SetCondition(aux.ArmorCondition)
	c:RegisterEffect(a2)
	--cannot attack
...
function s.filter(c)
	return c.is_armor and c:IsFaceup() and not c:IsCode(id)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_ONFIELD) and c35199656.filter(chkc) end
