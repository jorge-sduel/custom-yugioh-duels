...
function Auxiliary.AddRunicProcedure(c,f1,min1,max1,f2,min,max,loc)
	...
	e1:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_IGNORE_IMMUNE)
	e1:SetRange(loc)
	e1:SetCondition(Auxiliary.RunicCondition(e,c,min))
	e1:SetOperation(Auxiliary.RunicOperation(e,tp,eg,ep,ev,re,r,rp,c,min1,max1,min,max))
    e1:SetValue(SUMMON_TYPE_RUNIC)
	c:RegisterEffect(e1)
	--synchro custom
	local e2=Effect.CreateEffect(c)
...
function Auxiliary.RunicCondition(e,c,min)
	if c==nil then return true end
	return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>-1 and Duel.IsExistingMatchingCard(Runic.runfilter1,c:GetControler(),LOCATION_MZONE,0,min,nil)
end
function Auxiliary.RunicOperation(e,tp,eg,ep,ev,re,r,rp,c,min1,max1,min,max)
	...
	g:Merge(mt1)
	g2:Sub(mt1)
	local mt2=g2:Select(tp,min,max,nil)
	g:Merge(mt2)
	c:SetMaterial(g)
	Duel.SendtoGrave(g,REASON_MATERIAL+REASON_RUNIC)
end
